...
```
19. `impl Fn() -> ()` must be `+ Send + 'static` just to be called and it must be `impl Fn() -> Pin<Box<dyn std::future::Future<Output = () + Send + 'static>>> + Send + Sync + 'static` to be awaitable, i.e. `Sync` on top of `Send + 'static`
